...
- Emits SFD, OGN, and UNL pools and pack collation config as Lua.
...
def emit_pack_collations(setcodes: list[str]) -> str:
    lines = ["pack_collations = {"]
    for setcode in setcodes:
        lines.extend(
            [
                f"  {setcode} = {{",
                f'    {{ type = "{setcode}_Common_Pool", count = 7 }},',
                f'    {{ type = "{setcode}_Uncommon_Pool", count = 3 }},',
                f'    {{ type = "{setcode}_RarePlus_Roll", count = 2 }},',
                f'    {{ type = "{setcode}_Any_Roll", count = 1 }},',
                "  },",
            ]
        )
    lines.append("}")
...
def main() -> int:
    ...
    set_pools = [(setcode, bucket_rows(rows, set_prefix=setcode, set_name=set_name)) for setcode, set_name in SET_DEFS]

    lines: list[str] = []
    lines.append("-- Auto-generated by scripts/build_riftbound_collation.py")
    lines.append(f"-- Source: {args.cardlibrary}")
    lines.append("")
    for index, (setcode, pools) in enumerate(set_pools):
        lines.append(emit_set_block(setcode, pools))
        if index != len(set_pools) - 1:
            lines.append("")
    lines.append("")
    lines.append(emit_pack_collations([setcode for setcode, _ in set_pools]).rstrip())
    lines.append("")
    for setcode, pools in set_pools:
        lines.append(
            f"-- Counts: {setcode} C/U/R/E/S = {len(pools['COMMON'])}/{len(pools['UNCOMMON'])}/{len(pools['RARE'])}/{len(pools['EPIC'])}/{len(pools['SHOWCASE'])}"
        )
    lines.append("")
    lines.append(emit_runtime_footer().rstrip())
    out_text = "\n".join(lines) + "\n"
